...
if RUBY_PLATFORM.match?(/(win|w)(32|64)$/)
  begin
...
#
class IO
  def each_with_timeout(pid, seconds, sep_string = $INPUT_RECORD_SEPARATOR)
    ...
    check_update_thread = Thread.new do
      loop do
        sleep 0.1
        if last_update - Time.now < -seconds # rubocop:disable Style/IfUnlessModifier
